...
We can do so by setting the `untreated_tag` identifier like `set_identifier("untreated_tag" = c("MY_CONTROL_TERMINOLOGY_HERE"))`.
and specifying the `nested_keys` argument within `create_SE`.
...
## Pulling project Summarized Experiment object
The current approach uses AWS as the main source of all the processing data. `gDRcomponents` package is the main package for managing all the cached data and making the connection with AWS. Before starting with this package, make sure, that all the AWS-dependent variables are defined as environmental variables. 

The basic usage of this package for pulling down Summarized Experiments objects requires using three basic functions.

### `getProjects("aws")`
This function returns data.table with the information about projects that are currently stored on AWS. Column `project_id` can be used as an identifier for projects that can be  pulled using `pullProjectSE` function.

### `getMetaProjects("aws")`
This function returns data.table with meta projects to which projects belong. The conception of meta projects allows us to group multiple projects into categories. Please note that one project might belong to multiple meta projects.

### `pullProjectSE(project_id, "aws")`
The core function for pulling SummarizedExperiments of projects based on their project_id (output of `getProjects()`).

Note: Some core/testing data are stored in the indendepndent storage (called fixed data). If you want to access them, use parameter `fixed = TRUE` in functions `getProjects()` and `getMetaProjects()`.
