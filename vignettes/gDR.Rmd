...
Steps (2) and (3) can be combined into a single step through a convenience function: `runDrugResponseProcessingPipeline`. This function also split the data based on its types -- single-agent, combo, etc. Moreover, the output is `MultiAssayExperiment` object with each experiments representing different data type.
