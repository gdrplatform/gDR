...
# Import data
imported_data <- gDR::import_data(manifest, template, raw_data)
head(imported_data)
```
...
```{r}
inl <- gDRcore::prepare_input(imported_data)
detected_data_types <- names(inl$exps)
detected_data_types
se <- create_and_normalize_SE(imported_data, data_type = "single-agent", nested_confounders = "Barcode")
se
```
...
convert_se_assay_to_dt(mae[["single-agent"]], "Metrics")
```

# Saving project to local or AWS instance and add it to metaproject

One can use the project and metaproject concepts to facilitate managing the datasets. Each dataset can be pushed as a project belonging to the given metaproject.

In the first step, one has to define configuration data specific to his/her environment. The simplest way for this is to create a config from the template with `save_config_template`, modify it as needed, and load with `load_config`.
...
# modify the config saved in the 'config_path' to fit your env requirements
# here: path to shared dir has been updated
cfg <- yaml::read_yaml(config_path)
cfg$SHARED_DIR <- config_dir
...
In the second step, one has to specify the meta-project to which project with the new dataset will be added. One can create the new meta-project by defining the named list, with `Project name` and `Description` and using `add_metaproject_to_metaprojects_list`. Alternatively one can use `getMetaProjects` function to find the `metaprojectId` of interest.

If there are no mata-projects already, the proper warning will be triggered, and a new list of meta-projects will be created.

```{r, echo=TRUE}
metaproject_matadata <- list(`Project name` = "Testing metaproject", Description = "Metaproject containing testing data")

add_metaproject_to_metaprojects_list("local", metaproject_matadata)
```

Now we can push project using `pushProject` function, as output we will get id of new project

```{r, echo=TRUE}
project_id <- pushProject(source = "local", mae = mae, metaprojectId = 1, description = "new project")
project_id
```

If necessary, we can edit the added project, to add new MAE for existing project we can use function `save_MAE`

```{r, echo=TRUE}
save_MAE(source = "local", mae = mae, projectId = project_id)
```

We can check list of projects and metaprojects using functions `getProjects` and `getMetaProjects`

```{r, echo=TRUE}
getProjects("local")
```
```{r, echo=TRUE}
getMetaProjects("local")
```

To load project we can use function `pullProjectSE`

```{r, echo=TRUE}
pullProjectSE(source = "local", project_id = project_id)
```
