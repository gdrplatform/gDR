...
When we prepare MultiAssayExperiment we can prepare new project and add it to metaproject. Firstly we need to prepare new metaproject if we do not have any, or we can one of allready prepared. 

Firstly we need to define config with information about storing data. We can prepare our own basing on template from gDRcomponets using functions `save_config_template` and `load_config`
...
To prepare metaproject we need to have metadata of its. We define the as named list, with `Project name` and `Description`. Note that if we do not have any mataproject already, we will get warning about it, and function will prepare new list of metaprojects, with only our new prepared 
...
Now we can add project to proejct list and assign to metaproject

```{r, echo=TRUE}
project_id <- 1

add_project_to_projects_list("local", mae, projectId = project_id)
add_project_to_metaproject(source = "local", metaprojectId = 1, projectId = project_id)
```

The last step will be save our project 

```{r, echo=TRUE}
save_MAE(source = "local", mae = mae, project_id = project_id) 
```
